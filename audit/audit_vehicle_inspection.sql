-- Tabla auditora para la tabla VehicleInspection
CREATE TABLE audit_vehicle_inspection (
...
-- Función de auditoría para la tabla VehicleInspection
CREATE OR REPLACE FUNCTION log_vehicle_inspection_changes()
RETURNS TRIGGER AS $$
BEGIN
    IF (TG_OP = 'INSERT') THEN
        INSERT INTO audit_vehicle_inspection (
...
-- Trigger para auditar cambios en la tabla VehicleInspection
CREATE TRIGGER trg_audit_vehicle_inspection
AFTER INSERT OR UPDATE OR DELETE ON VehicleInspection
FOR EACH ROW EXECUTE FUNCTION log_vehicle_inspection_changes();
